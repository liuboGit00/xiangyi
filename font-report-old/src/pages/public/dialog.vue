...
export default {
  props: ["dialogOpt", "itemvalue", "reportmodel"],
  mounted() {
  ...
  data() {
    var validatePass = (rule, value, callback) => {
      ...
      if (value === "" || value === undefined) {
        ...
        callback(new Error("请输入长度在20以内的标题"));
      } else {
        callback();
      }
    };
    var validateNext = (rule, value, callback) => {
      //表单验证
      if (value === "" || value === undefined) {
        callback(new Error("输入内容不能为空"));
      } else if (value.length > 200) {
        callback(new Error("请输入长度在200以内的标题"));
      } else {
        callback();
      }
    };
    return {
      ...
      rules2: {
        ...
        // remark: [{required: true,validator: validateNext, trigger: "blur" }]
      }
    };
  },
  watch: {
    reportmodel() {
      this.form.reportTitle = this.reportmodel.reportTitle;
      // this.form.remark = this.reportmodel.remark;
      this.form.remark = '';
    }
  },
  computed: {
  ...
  methods: {
    ...
    dialogHandle: function(message, type) {
      ...
      this.form = {
        //保存的表单
        reportTitle: this.reportmodel.reportTitle,
        remark: this.reportmodel.remark,
        usefulTime1: "",
        usefulTime2: "",
        urgentType: "3",
        allowTwice: "1"
