  ...
  methods: {
    onSubmit() {
      ...
      if (self.form.account == "" || self.form.account == undefined) {
        ...
        return false;
      } else if (self.form.account != "" && names.test(self.form.account)==false) {
        self.$message({
        ...
        return false;
      } else if (self.form.password != "" && passwords.test(self.form.password)==false) {
        self.$message({
        ...
        server.login(self.form, function success(res) {
          ...
          if (res.data.infoCode == 0) {
            ...
            sessionStorage.setItem("testUserSessions", res.data.data.sessionId);
            self.$router.push({ path: "/homes" });
          } else {
            self.$message({
