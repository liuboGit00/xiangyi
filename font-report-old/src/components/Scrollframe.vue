...
export default {
  ...
  updated() {
    ...
    let offsetTop =
      self.$el.children.length > 0 ? self.$el.children[0].offsetTop : "250"; //第一条数据距离上部的距离
    let listHeight =
      self.$el.children.length > 0 ? self.$el.children[0].offsetHeight : 0; //单个元素高度
    let listTop =
      self.$el.children.length > 0 ? self.$el.children[0].offsetTop : offsetTop; //距离顶部高度
